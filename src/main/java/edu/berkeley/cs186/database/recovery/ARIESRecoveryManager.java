...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        // TODO(proj5): implement
        TransactionTableEntry tableEntry = transactionTable.get(transNum);
        LogRecord updateRecord = new UpdatePageLogRecord(transNum, pageNum, tableEntry.lastLSN, pageOffset, before, after);
        long LSN = logManager.appendToLog(updateRecord);
        tableEntry.lastLSN = LSN;
    ...
    public synchronized void checkpoint() {
        ...
        Iterator<Map.Entry<Long, TransactionTableEntry>> txnIter = transactionTable.entrySet().iterator();

        while (dptIter.hasNext()) {
            Map.Entry<Long, Long> entry = dptIter.next();
            chkptDPT.put(entry.getKey(), entry.getValue());

            if (!EndCheckpointLogRecord.fitsInOneRecord(chkptDPT.size(), chkptTxnTable.size())) {
                chkptDPT.remove(entry.getKey());
        ...
        while (txnIter.hasNext()) {
            Map.Entry<Long, TransactionTableEntry> entry = txnIter.next();
        ...
        while (iter.hasNext()) {
            ...
            if (transNumOpt.isPresent()) {
                ...
                if (txnEntry == null) {
                    ...
                    txnEntry = transactionTable.get(transNum);
                }
                txnEntry.lastLSN = logRecord.getLSN();
            }
            ...
            Optional<Long> pageNumOpt = logRecord.getPageNum();
            if (pageNumOpt.isPresent()) {
                long pageNum = pageNumOpt.get();
                switch (logRecord.getType()) {
                    case UPDATE_PAGE:
            ...
            if (logRecord.getType() == LogType.END_CHECKPOINT) {
                ...
                for (Map.Entry<Long, Pair<Transaction.Status, Long>> entry : chkptTxnTable.entrySet()) {
                    ...
                    txnEntry.lastLSN = Math.max(txnEntry.lastLSN, lastLSN);

                    Transaction.Status currStatus = txnEntry.transaction.getStatus();
                    if (currStatus == Transaction.Status.COMPLETE) {
                        continue;
                    }
                    if (status == Transaction.Status.ABORTING && currStatus == Transaction.Status.RUNNING) {
                        txnEntry.transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
                    } else if (status == Transaction.Status.COMMITTING && currStatus == Transaction.Status.RUNNING) {
                        txnEntry.transaction.setStatus(Transaction.Status.COMMITTING);
                    }
                }
            }
        }
        for (TransactionTableEntry txnEntry : new ArrayList<>(transactionTable.values())) {
            Transaction.Status status = txnEntry.transaction.getStatus();
            long transNum = txnEntry.transaction.getTransNum();
    ...
    void restartRedo() {
        ...
        Iterator<LogRecord> iter = logManager.scanFrom(startingLSN);
        while (iter.hasNext()) {
            LogRecord logRecord = iter.next();
            ...
            if (type == LogType.ALLOC_PART || type == LogType.UNDO_FREE_PART
                    || type == LogType.FREE_PART || type == LogType.UNDO_ALLOC_PART) {
    ...
    void restartUndo() {
        ...
        for (TransactionTableEntry txnEntry : transactionTable.values()) {
            if (txnEntry.transaction.getStatus() == Transaction.Status.RECOVERY_ABORTING) {
                pq.add(new Pair<>(txnEntry.lastLSN, txnEntry));
            }
        }
        ...
        while (!pq.isEmpty()) {
            ...
            TransactionTableEntry txnEntry = pair.getSecond();
            long transNum = txnEntry.transaction.getTransNum();
            LogRecord logRecord = logManager.fetchLogRecord(LSN);

            if (logRecord.isUndoable()) {
                LogRecord clr = logRecord.undo(txnEntry.lastLSN);
                long clrLSN = logManager.appendToLog(clr);
                txnEntry.lastLSN = clrLSN;
                clr.redo(this, diskSpaceManager, bufferManager);
            }
            ...
            if (nextLSN == 0L) {
                ...
                pq.add(new Pair<>(nextLSN, txnEntry));
            }
        }
    }
